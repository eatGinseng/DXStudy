...
bool ModelClass::InitializeBuffers(ID3D11Device* device)
{
	...
	//// 이제 인덱스 버퍼, 인덱스 카운트 대신 인스턴스 버퍼, 인스턴스 카운트를 사용할 것임.
	...
	// instance buffer array. Temporary.
	...
	instances[0].position = XMFLOAT3(0.0f, 0.0f, -2.0f);
	instances[1].position = XMFLOAT3(0.0f, 0.0f, -1.0f);
	instances[2].position = XMFLOAT3(0.0f, 0.0f, 0.0f);
	instances[3].position = XMFLOAT3(0.0f, 0.0f, 1.0f);

	// Set up the description of the instance buffer. Exactly same as a vertex buffer description.
...
{
	// Instance buffer는 다른 데이터를 가진 2번째 vertex buffer와도 같다. offset과 stride를 두 개 생성해서 배열로 보낸다.
