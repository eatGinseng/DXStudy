...
bool ModelClass::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, char* textureFilename1, char* normalTextureFilename, HWND hwnd, char* modelFilename)
{
	bool result;
	...
	{
		result = LoadNormalTexture(device, deviceContext, normalTextureFilename, hwnd);
		if (!result)
		{
			MessageBox(hwnd, L"Could not Load Normal Texture.", L"Error", MB_OK);
			return false;
		}
	}
...
{
	if (m_normalTexture->GetTexture() != NULL)
	{
...
bool ModelClass::InitializeBuffers(ID3D11Device* device)
{
	...
	//// 이제 더이상 여기에서 버텍스 / 인덱스를 직접 셋하지 않을 것임. 참고
...
bool ModelClass::LoadNormalTexture(ID3D11Device* device, ID3D11DeviceContext* deviceContext, char* filename1, HWND hwnd)
{
	bool result;
	...
	// Initialize the texture object.
	result = m_normalTexture->Initialize(device, deviceContext, filename1, hwnd);
	if (!result)
	{
		MessageBox(hwnd, L"Could not initialize the Texture object.", L"Error", MB_OK);
		return false;
...
void ModelClass::ReleaseTexture()
{
	...
	// Release the normal texture object.
	if (m_normalTexture)
	{
