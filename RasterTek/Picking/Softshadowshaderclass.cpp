...
bool SoftShadowShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"softshadow.vs", L"softshadow.ps");
	if (!result)
	{
		return false;
	}
...
bool SoftShadowShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix,
	...
	XMFLOAT3 lightPosition, XMVECTOR ambientColor, XMVECTOR diffuseColor)
{
	bool result;
	...
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture, shadowTexture, lightPosition, ambientCol,
		diffuseCol);
	if (!result)
	{
		return false;
...
bool SoftShadowShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, LPCWSTR vsFilename, LPCWSTR psFilename)
{
	HRESULT result;
...
void SoftShadowShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, LPCWSTR shaderFilename)
{
	char* compileErrors;
