...
// 먼저 파티클에 사용될 텍스처를 로드한다. 로드 한 뒤에 particle system을 초기화한다.
// 초기화 된 다음에는, 빈 Vertex buffer와 index buffer를 만든다. 아직 파티클이 생성되지 않았기 때문에..
bool ParticleSystemClass::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, char* textureFilename, HWND hwnd)
{
	bool result;
...
// Frame 함수에서는 대부분의 particle system 이 동작하는 부분이다.
// 매 프레임마다 render life의 끝에 도달한 파티클들을 clear 한다.
// 필요한 타이밍일 경우 새로운 파티클을 생성한다. 생성한 다음에는 현재 생성된 파티클들을 모두 업데이트 한다.
// 이 예제에서는 떨어지는 듯한 효과를 위해 파티클의 height position을 업데이트 할 것이다.
// 파티클이 업데이트 되었다면, 각 파티클의 업데이트 된 위치로 vertex buffer를 업데이트 해 준다.
// vertex buffer가 dynamic이기 때문에 이 과정은 쉽다.
...
bool ParticleSystemClass::LoadTexture(ID3D11Device* device, ID3D11DeviceContext* deviceContext, char* filename, HWND hwnd)
{
	bool result;
...
// 파티클 프로퍼티에 사용되는 여러 요소들을 초기화 한다.
// 여기에서는 파티클이 생성되는 위치를 랜덤하게 지정할 것이다.
// 또한 떨어지는 속도도 각 파티클마다 랜덤하게 지정할 것이다.
// 그 이후에 파티클의 크기, 매 초마다 파티클이 몇 개나 생성될 지, 그리고 이 시스템에서 한번에 몇 개의 파티클을 허용할 것인지도 결정한다.
...
// EmitParticles 함수에서는 언제 파티클을 생성할 것인지 결정한다.
// 프레임 타임과 Particles per second 를 보고 결정하게 된다.
// 만약 생성될 파티클이 있다면, 새로운 파티클이 생성되고, 각각의 프로퍼티들이 셋팅된디ㅏ.
// 그리고 나서 새로운 파티클들은 파티클 array의 z depth order 에 삽입된다.
// Particle array는 올바른 깊이 순서로 소팅되어야 alpha blend 를 사용해서 올바르게 렌더링 될 수 있다.
...
// UpdateParticles에서는 매 프레임 파티클의 property들을 업데이트한다.
// particle의 speed에 따라 height position을 업데이트한다.
...
// KillParticle 함수에서는 렌더링 라이프타임의 끝에 도달한 파티클을 System으로부터 분리한다.
// 이 함수는 매 프레임 호출되어 없어져야 할 파티클들이 있는지 살펴본다. 여기에서는 height가 -3.0 이하인지 체크해서
// 해당 파티클을 삭제하고, array는 depth order 에서 뒤쪽으로 이동한다.
...
// updateBuffers 함수는 매 프레임 호출되어 dynamic vertex buffer 전체를 업데이트 된 파티클 시스템 내 파티클 전체의 위치로 업데이트한다.
bool ParticleSystemClass::UpdateBuffers(ID3D11DeviceContext* deviceContext)
{
	...
	for (i = 0; i < m_currentParticleCount; i++)
	{
		...
		m_vertices[index].color = XMVectorSet(m_particleList[i].red, m_particleList[i].green, m_particleList[i].blue, 1.0f);
		index++;
