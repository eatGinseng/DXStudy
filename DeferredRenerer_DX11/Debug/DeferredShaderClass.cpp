...
bool DeferredShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"deferred.vs", L"deferred.ps");
	if(!result)
	{
		return false;
	}
...
bool DeferredShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix,
	XMMATRIX projectionMatrix, XMMATRIX lightViewMatrix, XMMATRIX lightProjectionMatrix, ID3D11ShaderResourceView* texture)
{
	bool result;
	...
	XMMATRIX projectionMatrix, XMMATRIX lightViewMatrix, XMMATRIX lightProjectionMatrix, ID3D11ShaderResourceView* texture)
{
	...
	lightProjectionMatrix = XMMatrixTranspose(lightViewMatrix);
	...
	dataPtr2->lightProjectionMatrix = lightProjectionMatrix;
